...
# from utils.tone_engine import detect_tone
# from utils.recommendation_engine import generate_recommendations, rewrite_email
...
class EmailAnalysisService:
    def analyze(self, raw_text: str) -> EmailAnalysisResult:
        ...
        # ── Step 5: Tone detection + Recommendations ────────────────────
        # Uses raw_text so greetings/sign-offs contribute to tone scoring.
        # These calls are isolated — they cannot affect Steps 1-4 above.
        tone             = detect_tone(raw_text)
        recommendations  = generate_recommendations(raw_text, tone)
        improved         = rewrite_email(raw_text)

        logger.info(
            "Result — intent=%s sentiment=%s urgency=%s risk=%s department=%s tone=%s",
            intent, sentiment, urgency, risk, department, tone,
        )
